...
\begin{document}
\begin{align*}
  \mathbf{type}\ E_a = [(\hat{T}, a)] \text{ -- for non-decreasing times where } a \text{ is not } E_a
\end{align*}
\begin{align*}
  \mathbf{type}\ E_{E_a} = [(\hat{T}_{outer}, [\hat{T}_{inner}, a])] \text{ -- for non-decreasing times where } \hat{T}_{outer} \leq \hat{T}_{inner}
\end{align*}
\begin{align*}
